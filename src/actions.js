// Action timer engine, unlock/milestone checks, upgrade purchase, manager hiring.

import { state } from './state.js';
import { ACTIONS, UPGRADES_DEF, MILESTONES_DEF, TITLES, COOK_IDS, MAX_MEAL_STOCK, applyMilestoneRewards } from './data.js';
import { SFX } from './audio.js';
import { notify } from './ui.js';
import { applyPrestigeMultipliers, getPrestigeTier, getViralThreshold, calcCloutEarned } from './prestige.js';
import {
  renderActions, renderActionCard, updateRunningState, renderUpgrades, renderMilestones,
  renderStaffTab, updateStaffButtons, updateUpgradeGlow, updatePrestigePreview,
  updateTabBadges, updateStats, fmtMoney, buildActionCard,
} from './render.js';
import { stallUnlocked, getMealFreshness, renderStall } from './stall.js';

export function isActionUnlocked(a) {
  const G = state.G;
  const u = a.unlockAt;
  if (u.level && G.level < u.level) return false;
  if (u.subs && G.subs < u.subs) return false;
  if (u.money && G.money < u.money) return false;
  if (u.totalCooks && G.totalCooks < u.totalCooks) return false;
  if (u.totalFilmed && G.totalFilmed < u.totalFilmed) return false;
  if (u.totalEdits && G.totalEdits < u.totalEdits) return false;
  return true;
}

export function isUpgradeUnlocked(u) {
  const G = state.G;
  const r = u.unlockAt;
  if (r.level && G.level < r.level) return false;
  if (r.subs && G.subs < r.subs) return false;
  if (r.money && G.money < r.money) return false;
  if (r.totalEdits && G.totalEdits < r.totalEdits) return false;
  if (r.totalFilmed && G.totalFilmed < r.totalFilmed) return false;
  return true;
}

export function getActionDuration(a) {
  return Math.max(500, a.baseTime / state.G.mx.speed);
}

export function startAction(id) {
  if (!state.G.actionTimers[id]) state.G.actionTimers[id] = { progress: 0, running: false };
  const st = state.G.actionTimers[id];
  if (st.running) return;
  st.running = true;
  runActionLoop(id);
}

export function runActionLoop(id) {
  const action = ACTIONS.find((a) => a.id === id);
  if (!action || !isActionUnlocked(action)) return;
  const st = state.G.actionTimers[id];
  if (!st || !st.running) return;

  const duration = getActionDuration(action);
  const startTime = Date.now() - st.progress * duration;

  function tick() {
    if (!state.G.actionTimers[id] || !state.G.actionTimers[id].running) {
      delete state.activeTimers[id];
      return;
    }
    const elapsed = Date.now() - startTime;
    const pct = Math.min(1, elapsed / duration);
    state.G.actionTimers[id].progress = pct;

    const bar = document.getElementById('ap-' + id);
    if (bar) bar.style.width = pct * 100 + '%';

    if (pct >= 1) {
      if (bar) bar.style.width = '100%';
      const card = document.getElementById('ac-' + id);
      if (card) {
        card.classList.remove('action-complete-flash');
        void card.offsetWidth;
        card.classList.add('action-complete-flash');
        setTimeout(() => card.classList.remove('action-complete-flash'), 600);
      }
      if (action.id.startsWith('cook_')) SFX.cook();
      else if (action.id === 'film_clip') SFX.film();
      else SFX.complete();
      collectReward(action);

      const isCook = COOK_IDS.includes(id);
      const shouldLoop = isCook ? state.G.activeCookId === id : state.G.managers.includes(id);
      if (shouldLoop) {
        setTimeout(() => {
          if (!state.G.actionTimers[id]) return;
          state.G.actionTimers[id].progress = 0;
          state.G.actionTimers[id].running = true;
          runActionLoop(id);
        }, 250);
      } else {
        state.G.actionTimers[id].progress = 0;
        state.G.actionTimers[id].running = false;
        renderActionCard(id);
      }
      return;
    }
    state.activeTimers[id] = requestAnimationFrame(tick);
  }
  state.activeTimers[id] = requestAnimationFrame(tick);
}

export function collectReward(action) {
  const G = state.G;
  if (action.id.startsWith('cook_')) {
    G.totalCooks++;
    const stockYield = { cook_eggs: 1, cook_pasta: 2, cook_stirfry: 2, cook_sushi: 3, cook_wellington: 1, cook_ramen: 2 };
    const addCount = stockYield[action.id] || 1;
    if (G.mealQueue.length < MAX_MEAL_STOCK) {
      const freshness = getMealFreshness();
      const canAdd = Math.min(addCount, MAX_MEAL_STOCK - G.mealQueue.length);
      for (let i = 0; i < canAdd; i++) {
        G.mealQueue.push({
          id: ++state.mealIdCounter,
          addedAt: stallUnlocked() ? Date.now() : null,
          maxAge: freshness,
        });
      }
      G.mealStock = G.mealQueue.length;
      if (state.currentTab === 'stall') renderStall();
    }
  }
  if (action.id === 'film_clip') G.totalFilmed++;
  if (action.id === 'edit_video' || action.id === 'thumbnail') G.totalEdits++;

  const moneyBefore = G.money;
  action.reward(G);
  const earned = G.money - moneyBefore;
  if (earned > 0) G.money = moneyBefore + earned * G.mx.money;

  checkLevelUp();
  checkUnlocks();
  checkMilestones();
  renderActionCard(action.id);
}

export function checkLevelUp() {
  const G = state.G;
  let leveled = false;
  while (G.xp >= G.xpNeeded) {
    G.xp -= G.xpNeeded;
    G.level++;
    G.xpNeeded = Math.floor(G.xpNeeded * 1.45);
    SFX.levelUp();
    notify('LEVEL UP — now Level ' + G.level + '!', '');
    leveled = true;
  }
  if (leveled) {
    updateTitle();
    checkUnlocks();
  }
}

export function updateTitle() {
  let t = 'Aspiring Cook';
  for (const [lvl, name] of TITLES) if (state.G.level >= lvl) t = name;
  const p = state.G.prestiges > 0 ? ' (x' + state.G.prestiges + ' Viral)' : '';
  const el = document.getElementById('title-rank');
  if (el) el.textContent = '★ ' + t + p;
}

function insertUnlockedActionCard(action) {
  const isCook = COOK_IDS.includes(action.id);
  const colId = isCook ? 'kitchen-col' : 'content-col';
  const col = document.getElementById(colId);
  if (!col) {
    renderActions();
    return;
  }
  const existing = document.getElementById('ac-' + action.id);
  if (existing) existing.remove();
  const tmp = document.createElement('div');
  tmp.innerHTML = buildActionCard(action, false);
  const card = tmp.firstElementChild;
  col.appendChild(card);
  if (state.currentTab === 'staff') renderStaffTab();
  else updateStaffButtons();
}

export function checkUnlocks() {
  ACTIONS.forEach((a) => {
    if (isActionUnlocked(a) && !state.prevUnlockedActions.has(a.id)) {
      state.prevUnlockedActions.add(a.id);
      SFX.unlock();
      notify('🔓 Unlocked: ' + a.name, 'blue');
      insertUnlockedActionCard(a);
    }
  });
  UPGRADES_DEF.forEach((u) => {
    if (isUpgradeUnlocked(u) && !state.G.ownedUpgrades.includes(u.id) && !state.prevUnlockedUpgrades.has(u.id)) {
      state.prevUnlockedUpgrades.add(u.id);
    }
  });
  updateUpgradeGlow();
  if (state.currentTab === 'prestige') updatePrestigePreview();
  if (state.currentTab === 'staff') updateStaffButtons();
  updateTabBadges();
  const cpEl = document.getElementById('clout-preview');
  if (cpEl) {
    cpEl.style.display = state.G.subs >= getViralThreshold() * 0.4 ? 'block' : 'none';
    cpEl.textContent = 'Going viral now: ~' + calcCloutEarned() + ' CP';
  }
}

export function checkMilestones() {
  const G = state.G;
  let changed = false;
  MILESTONES_DEF.forEach((m) => {
    if (!G.doneMilestones.includes(m.id) && m.check(G)) {
      G.doneMilestones.push(m.id);
      SFX.milestone();
      notify('🏆 Milestone: ' + m.name, '');
      applyMilestoneRewards(G, m.id);
      changed = true;
    }
  });
  if (changed && state.currentTab === 'milestones') renderMilestones();
}

export function buyUpgrade(id) {
  const G = state.G;
  const u = UPGRADES_DEF.find((x) => x.id === id);
  if (!u || G.ownedUpgrades.includes(id) || G.money < u.cost || !isUpgradeUnlocked(u)) return;
  G.money -= u.cost;
  G.ownedUpgrades.push(id);
  u.effect(G);
  SFX.upgrade();
  notify('✦ Upgrade: ' + u.name, 'green');
  renderUpgrades();
  updateTabBadges();
  updateStats();
}

export function reapplyUpgrades() {
  const G = state.G;
  G.mx = { xp: 1, sub: 1, view: 1, speed: 1, money: 1 };
  G.ownedUpgrades.forEach((id) => {
    const u = UPGRADES_DEF.find((x) => x.id === id);
    if (u) u.effect(G);
  });
  G.doneMilestones.forEach((id) => applyMilestoneRewards(G, id));
  applyPrestigeMultipliers();
}

export function clickAction(id) {
  if (!state.G.actionTimers[id]) state.G.actionTimers[id] = { progress: 0, running: false };
  if (!state.G.actionTimers[id].running) {
    startAction(id);
    updateRunningState(id, true);
  }
}

export function buyManager(id) {
  const G = state.G;
  const a = ACTIONS.find((x) => x.id === id);
  if (!a || G.managers.includes(id) || COOK_IDS.includes(id)) return;
  const discTier = getPrestigeTier('pu_manager_disc');
  const discount = discTier >= 0 ? Math.pow(0.8, discTier + 1) : 1;
  const effectiveCost = Math.floor(a.managerCost * discount);
  if (G.money < effectiveCost) return;
  G.money -= effectiveCost;
  G.managers.push(id);
  SFX.manager();
  notify('🤖 Hired: ' + a.managerName, 'green');
  void fmtMoney; // no-op reference; preserved import for symmetry with original
  if (!G.actionTimers[id] || !G.actionTimers[id].running) {
    startAction(id);
  }
  renderActions();
  updateStats();
}

export function setActiveCook(id) {
  const prev = state.G.activeCookId;

  if (prev && prev !== id) {
    if (state.G.actionTimers[prev]) state.G.actionTimers[prev].running = false;
    if (state.activeTimers[prev]) {
      cancelAnimationFrame(state.activeTimers[prev]);
      delete state.activeTimers[prev];
    }
  }

  if (state.G.activeCookId === id) {
    state.G.activeCookId = null;
    if (state.G.actionTimers[id]) state.G.actionTimers[id].running = false;
    if (state.activeTimers[id]) {
      cancelAnimationFrame(state.activeTimers[id]);
      delete state.activeTimers[id];
    }
  } else {
    state.G.activeCookId = id;
    if (!state.G.actionTimers[id]) state.G.actionTimers[id] = { progress: 0, running: false };
    if (!state.G.actionTimers[id].running) startAction(id);
  }
  renderActions();
}
